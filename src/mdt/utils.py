...
def generate_module(rxcui_ndc_df, rxclass_name):
    ...
    for ingredient_name in medication_ingredient_list:
        filename = rxclass_name + '_product_' + ingredient_name + '_distrib'
        #0
        meps_rxcui_ingred = meps_rxcui[meps_rxcui['medication_ingredient_name']==ingredient_name][['medication_product_name',  'medication_product_rxcui', 'medication_ingredient_name',  'medication_ingredient_rxcui', 'person_weight', 'DUPERSID']+groupby_demographic_variables]
        #1
...
def generate_module_distribution_df(meps_rxcui_ndc_df):
